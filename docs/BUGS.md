...
## B10 — Implausible GPS fixes are trusted (2026-09-20) — FIXED 0.23.2 (`SaneLocationProvider`), verified 0.26.1
...
## B11 — Route replaced during a reroute comes back (2026-09-20) — FIXED 0.27.0 (`NavLock`)
FerrostarCore 0.56 has no lock between `onLocationUpdated` (Dispatchers.IO collector: read `_navState`, run the session, write the result back unconditionally) and `replaceRoute` (called from the reroute coroutine or the UI). First arrival test on the emulator: deviation → reroute to a 2-step 5 m route → "You have arrived" spoken → 350 ms later the discarded 11 mi route's state was written back (`stepIdx=1 … remaining=10.94mi steps=10`), and because the session now belonged to the short route the trip sat there forever with End still up. On the road this is the "keeps recalculating / still taking me home after End" family of symptoms (B1 was the same race on stop). Fix: every core mutation (`startNavigation`, `replaceRoute`, `stopNavigation`) and every fix delivered to the core go through one mutex (`nav/NavLock.kt`, `LockedLocationProvider`). Verified: `docs/emu-arrival.sh` run a2 rerouted 6 s before arrival and completed on the new route. Upstream: worth an issue against stadiamaps/ferrostar (`FerrostarCore.onLocationUpdated` vs `replaceRoute`).
