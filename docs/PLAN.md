...
| S25 | Relay board live state (ESPHome /events push) + Venus LAN fix + Pi Wi-Fi watchdog v2 | S | DONE 0.40.1 / 149 (claude-power); 0.40.0 turn-off 404 fixed, real-board on/off/on verified | `s25-relay.sh` run5 11/11, `s25-venus-lan.sh` 4/4, s24 11/11, cockpit 13/13 |
...
**S25b (0.40.1 / 149) — turn-off 404.** On the truck, 0.40.0 could switch relays on but not off: ESPHome's change events (DETAIL_STATE) carry only name_id/id/value/state, and `onState` overwrote the stored name with "", so the next command went to `/switch//turn_off` → 404. The fake board always sent `name`, so S25 missed it. Fix: name from `name_id` (in every event), key = ESPHome object id derived from the name (`id` changes format in ESPHome 2026.8). Fake board now sends brief change events like ESPHome; new row R9 (tap ×3 → board sees on/off/on). Reproduced on 0.40.0 (R9, R7b, R8 FAIL), 12/12 on 0.40.1; real board from the emulator: Relay 3 (unassigned) on → off → on, no 404.
