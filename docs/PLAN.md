...
2. Overlay: a raster layer above roads from the active provider's flow tiles (Google has no flow tiles → the Google option gives ETA only, and says so in Settings). Toggle in Layers; auto-hidden offline; tile requests counted in NavLog (`traffic tiles=…`) per day.
3. ETA: each A/B/C candidate gets a second number `· 19 min w/ traffic` from the provider (TomTom: routing with the same via points; Google: `computeRoutes` TRAFFIC_AWARE with the polyline's waypoints) when the answer arrives within 3 s; the trip bar's ETA switches to the traffic one while it is fresher than 10 min. Valhalla remains the router.
...
Done when (emulator `build01:emulator-5554`, key set via `PUT /api/settings`): (a) Layers → Traffic on → coloured flow segments over I-35W in a screenshot within 5 s; NavLog `traffic tiles=N` and N ≤ 2,000 for a 30-minute fake drive; (b) `svc wifi disable` on the *emulator* → overlay hidden, 0 requests, no crash; back online → returns within 10 s; (c) preview cards show `14 min · 19 min w/ traffic` when the provider answers within 3 s, plain when not, for both providers; (d) bad key → Settings "Test key" reports the HTTP status, nothing else breaks; (e) `grep -rn "AIza\|tomtom.*key" app/src` finds no literal keys; (f) `cockpit-smoke.sh` 13/13.

**S20 contract Astra codes against** (vehicle track ships it first, small commit, same day): `com.morton.trucknav.settings.Settings` object — `get(key: String): String?`, `set(key: String, value: String?)`, `flow(key): StateFlow<String?>`, backed by `files/settings.json`; `GET /api/settings` (values for secret keys masked to last 4), `PUT /api/settings {k: v, …}`; MCP `get_settings`, `set_setting(key, value)`. Keys used: `tomtomKey`, `googleMapsKey`, `trafficProvider`.
