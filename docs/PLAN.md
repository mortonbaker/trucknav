...
### S22 — Traffic (medium + API key) — NEW
Facts (checked 2026-09-20): self-hosted Valhalla has no live traffic without a feed; free/cheap sources for one truck: **TomTom** Traffic Flow raster/vector tiles (free allowance: 50 k tile requests/day per the pricing page; 200 k/month per its FAQ — either covers a personal truck) + TomTom Routing (2,500 non-tile requests/day free) gives an overlay *and* a traffic-aware ETA with one key; **Google Routes** `TRAFFIC_AWARE` is a Pro SKU: 5,000 free/month then $15/1000. Sources: [TomTom pricing](https://docs.tomtom.com/pricing), [TomTom flow tiles](https://developer.tomtom.com/traffic-api/documentation/traffic-flow/raster-flow-tiles), [Routes API billing](https://developers.google.com/maps/documentation/routes/usage-and-billing).
Recommendation: TomTom. Layer 1 = flow tiles as a raster layer above roads (online only, toggle in Layers, auto-off offline); Layer 2 = "with traffic" ETA next to each A/B/C candidate from TomTom routing for the same polyline (Valhalla stays the router; Tesla/Maps show one number — we show ours and the traffic one until we trust it); Layer 3 (later) = incidents as pins from the Incidents API.
Done when: (a) online, Layers → Traffic on → coloured flow segments visible over I-35W in a screenshot within 5 s, tile count in the day's NavLog ≤ 2,000 for a 30-minute drive; (b) offline → layer hidden, no requests, no crash; (c) preview cards show `14 min · 19 min w/ traffic` when TomTom answers within 3 s, plain when not; (d) key lives in Settings (S20), never in source; (e) 0 crashes.
...
Build: move Layers + Add-stop into the top-right stack under overview/mute; recenter into the bottom-right stack with zoom; keep 16 dp corner margins, 56 dp buttons, 12 dp gaps; portrait mirrors the same corners.
